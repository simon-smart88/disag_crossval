...
for (r in 1:n_reps){
  ...
  for (n in 1:n_groups){
    ...
    names(lm_train)[8] <- "response"
    lmod <- lm(response ~ population * LSTmean * LSTsd * EVI * Elevation, data = lm_train)
...
for (r in 1:n_reps){
  results <- as.data.frame(merge(response, out[out$rep == r,], by = "ID"))
